...
async function post(name,path,body,proxy=false) {
  ...
  await writeFile(new URL('crosscheck-now-responses.json',directory),JSON.stringify(results,null,2));
...
for(let i=0;i<2;i++) {
  ...
  const date={from:new Date(0).toISOString(),to:new Date().toISOString()};
  ...
  await post(`wallet-${i}-holdings-now`,'profiler/address/pnl',{
    ...source.body,date,filters:{show_realized:false},pagination:{page:1,per_page:1000},order_by:[{field:'holding_usd',direction:'DESC'}]
  });
  await post(`wallet-${i}-summary-now`,'profiler/address/pnl-summary',{...source.body,date});
}
