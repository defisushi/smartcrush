...
6. New-position/add/partial-sale/full-exit labels are **estimates** reconstructed backwards from current holdings and the recent trade window. Unknown or inconsistent data produces “Context unavailable.” Transfers and indexing delays can affect estimates. “Taking profit” means a partial sale, not a verified profitable sale.
7. Signal IDs include transaction hash, wallet, token, and action. This deduplicates repeated polls without discarding the opposite leg of a token-to-token swap. Up to 1,000 signals are retained, and a breakup removes that wallet’s signals, including results from a poll already in progress.
